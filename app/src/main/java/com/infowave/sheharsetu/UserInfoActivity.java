...
public class UserInfoActivity extends AppCompatActivity {
    ...
    /* ===== Google Translate ===== */
    private static final String G_TRANSLATE_URL =
            "https://translation.googleapis.com/language/translate/v2";
    // Per your request (for production, proxy on server):
    private static final String GOOGLE_TRANSLATE_API_KEY =
            "AIzaSyCkUxQSJ1jNt0q_CcugieFl5vezsNAUxe0";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        I18n.prefetch(this, staticKeys, () -> {
            ...
            if (rbVillage != null) rbVillage.setText(I18n.t(this, rbVillage.getText().toString()));
            if (rbCity != null) rbCity.setText(I18n.t(this, rbCity.getText().toString()));
    ...
    private boolean validateAll() {
        if (etName.getText() == null || textOf(etName).length() < 2) { ((TextInputLayout) etName.getParent().getParent()).setError(I18n.t(this, "Enter valid name")); etName.requestFocus(); return false; }
        if (etSurname.getText() == null || textOf(etSurname).length() < 2) { ((TextInputLayout) etSurname.getParent().getParent()).setError(I18n.t(this, "Enter valid surname")); etSurname.requestFocus(); return false; }
        if (etMobile.getText() == null || !textOf(etMobile).matches("^[6-9]\\d{9}$")) { ((TextInputLayout) etMobile.getParent().getParent()).setError(I18n.t(this, "Enter 10-digit Indian mobile")); etMobile.requestFocus(); return false; }
        if (TextUtils.isEmpty(spState.getText())) { ((TextInputLayout) spState.getParent().getParent()).setError(I18n.t(this, "Select state")); spState.requestFocus(); return false; }
        if (TextUtils.isEmpty(spDistrict.getText())) { ((TextInputLayout) spDistrict.getParent().getParent()).setError(I18n.t(this, "Select district")); spDistrict.requestFocus(); return false; }
        if (etPlaceName.getText() == null || textOf(etPlaceName).length() < 2) { ((TextInputLayout) etPlaceName.getParent().getParent()).setError(I18n.t(this, "Enter village/city")); etPlaceName.requestFocus(); return false; }
        if (etAddress.getText() == null || textOf(etAddress).length() < 5) { ((TextInputLayout) etAddress.getParent().getParent()).setError(I18n.t(this, "Enter full address")); etAddress.requestFocus(); return false; }
        if (etPincode.getText() == null || !textOf(etPincode).matches("^\\d{6}$")) { ((TextInputLayout) etPincode.getParent().getParent()).setError(I18n.t(this, "Enter 6-digit pincode")); etPincode.requestFocus(); return false; }
        if (!cbTerms.isChecked()) { showToast(I18n.t(this, "Please accept Terms & Conditions")); return false; }
        // clear errors
    ...
    private void setSending(boolean v) {
        ...
        if (v) { btnContinue.setText(I18n.t(this, "Loading…")); showLoading(I18n.t(this, "Loading…")); }
        else { btnContinue.setText(I18n.t(this, "Continue")); hideLoading(); }
    }
    ...
    private void sendOtp(String mobile) {
        ...
        try {
            ...
            JsonObjectRequest req = new JsonObjectRequest(Request.Method.POST, ApiRoutes.SEND_OTP, body,
                    ...
                    err -> { setSending(false); showToast(I18n.t(this, readableVolleyError(err))); }
            ){
        ...
        try {
            ...
            JsonObjectRequest req = new JsonObjectRequest(Request.Method.POST, ApiRoutes.VERIFY_OTP, body,
                    resp -> {
                        ...
                        boolean ok = resp.optBoolean("ok", false);
                        if (!ok) { setOtpInlineError(I18n.t(this, "Invalid/expired OTP")); return; }
                        ...
                        if (access != null && refresh != null && userId > 0 && expiresIn > 0) {
                            ...
                            if (isNew) saveProfileThenGoHome();
                            else {
                    ...
                    err -> { isVerifyingOtp = false; setOtpInlineError(I18n.t(this, "Network error")); }
            ){
                @Override public java.util.Map<String, String> getHeaders() throws AuthFailureError {
                    HashMap<String, String> h = new HashMap<>();
                    h.put("Content-Type", "application/json; charset=utf-8");
    ...
    private void saveProfileThenGoHome() {
        ...
        try {
            ...
            if (checkedId == R.id.rbVillage) placeType = "village";
            else if (checkedId == R.id.rbCity) placeType = "city";

            if (isBlank(full)) { isSavingProfile = false; hideLoading(); ((TextInputLayout) etName.getParent().getParent()).setError(I18n.t(this, "Enter valid name")); etName.requestFocus(); return; }
            if (!phone.matches("^[6-9]\\d{9}$")) { isSavingProfile = false; hideLoading(); ((TextInputLayout) etMobile.getParent().getParent()).setError(I18n.t(this, "Enter 10-digit Indian mobile")); etMobile.requestFocus(); return; }
            if (!isBlank(pin) && !pin.matches("^\\d{6}$")) { isSavingProfile = false; hideLoading(); ((TextInputLayout) etPincode.getParent().getParent()).setError(I18n.t(this, "Enter 6-digit pincode")); etPincode.requestFocus(); return; }
            ...
            if (placeType != null) body.put("place_type", placeType);
            ...
            JsonObjectRequest req = new JsonObjectRequest(Request.Method.POST, ApiRoutes.SAVE_PROFILE, body,
                    ...
                    err -> { isSavingProfile = false; hideLoading(); showToast(I18n.t(this, readableVolleyError(err))); }
            ){
                @Override public java.util.Map<String, String> getHeaders() throws AuthFailureError {
                    HashMap<String, String> h = new HashMap<>();
                    h.put("Content-Type", "application/json; charset=utf-8");
    ...
    private void showOtpSheet(String mobile) {
        ...
        tvResend.setOnClickListener(v -> {
            ...
            d1.requestFocus();
            startResendTimer(tvTimer, tvResend);
            if (!isSendingOtp) sendOtp(mobile);
        });
    ...
    private void setOtpInlineError(String msg){
        if (otpDialog == null) return;
        TextView tvTimer = otpDialog.findViewById(R.id.tvTimer);
        if (tvTimer != null) tvTimer.setText(msg);
    }
    ...
    private static String textOf(TextInputEditText e) { return e.getText()==null ? "" : e.getText().toString().trim(); }
    private static boolean isBlank(String s) { return s == null || s.trim().isEmpty(); }
    private static void jsonPutIfNotBlank(JSONObject o, String key, String val) throws JSONException { if (val != null) { String t = val.trim(); if (!t.isEmpty()) o.put(key, t); } }
    private String firstNonEmpty(String a, String b) { return (a != null && !a.isEmpty()) ? a : (b == null ? "" : b); }
    ...
    private String readableVolleyError(VolleyError err) {
        ...
        if (nr != null) {
            ...
            String body;
            try { body = new String(nr.data, StandardCharsets.UTF_8); } catch (Exception e) { body = ""; }
            try {
                JSONObject o = new JSONObject(body);
                String msg = firstNonEmpty(o.optString("message","").trim(), o.optString("error","").trim());
                String codeStr = o.optString("error_code", "");
                if ("ALREADY_REGISTERED".equalsIgnoreCase(codeStr)) return "You are already registered. Please login.";
                if ("NOT_REGISTERED".equalsIgnoreCase(codeStr))     return "This number is not registered.";
                if (!msg.isEmpty()) return "HTTP " + code + " - " + msg;
            } catch (Exception ignored) {}
            if (code >= 500) return "Server unavailable. Please try again.";
            return "HTTP " + code;
        }
    ...
    private void showLoading(String message) {
        ...
        if (tv != null) tv.setText(message == null ? I18n.t(this, "Loading…") : message);
    ...
    private void showToast(String msg) {
        ...
        if (singleToast == null) singleToast = Toast.makeText(this, msg, Toast.LENGTH_SHORT);
        else { singleToast.setText(msg); singleToast.setDuration(Toast.LENGTH_SHORT); }
        singleToast.show();
    }
    ...
    static class SimpleTextWatcher implements TextWatcher {
        ...
        private final OnChange cb; SimpleTextWatcher(OnChange c){ this.cb = c; }
                ...
                resp -> {
                    try {
                        ...
                        for (int i = 0; i < arr.length() && i < needNetwork.size(); i++) {
                            ...
                            if (pos != null) result.set(pos, plain);
                        ...
                        for (int i = 0; i < result.size(); i++) {
                            if (result.get(i) == null) result.set(i, english.get(i));
                        }
                        onSuccess.accept(result);
                    } catch (Exception e) {
                        if (onError != null) onError.run();
